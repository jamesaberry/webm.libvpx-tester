...
int vp8_multi_resolution_encoder(int argc, char **argv)
//int vp8_multi_resolution_encoder(long width, long height, const char *input, const char *output[3])
{
    ...
    /* Open input video file for encoding */
    //if(!(infile = fopen(input, "rb")))
    //    die("Failed to open %s for reading", input);
    ...
    //for (i=0; i< NUM_ENCODERS; i++)
    //{
    //    if(!(outfile[i] = fopen(output[i], "wb")))
    //        die("Failed to open %s for writing", output[i]);
    //}
