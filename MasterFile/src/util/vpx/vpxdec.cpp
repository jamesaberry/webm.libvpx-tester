...
void die_ivfdec(const char *fmt, ...)
{
    va_list ap;
    va_start(ap, fmt);
...
static int read_frame_vpxdec(struct input_ctx      *input,
                             uint8_t               **buf,
                             size_t                *buf_sz,
                             size_t                *buf_alloc_sz)
{
...
void *out_open_ivfdec(const char *out_fn, int do_md5)
{
    void *out = NULL;
    ...
    else
    {
        ...
        out = strcmp("-", out_fn) ? fopen(out_fn, "wb") : stdout;
        FILE *outfile = (FILE *)out;
...
void out_put_ivfdec(void *out, const uint8_t *buf, unsigned int len, int do_md5)
{
    if (do_md5)
    {
#if CONFIG_MD5
    ...
    {
        if (fwrite(buf, 1, len, (FILE *)out));
    }
}

void out_close_ivfdec(void *out, const char *out_fn, int do_md5)
{
    if (do_md5)
    {
#if CONFIG_MD5
...
unsigned int file_is_raw_ivfdec(FILE *infile,
                                unsigned int *fourcc,
                                unsigned int *width,
                                unsigned int *height,
                                unsigned int *fps_den,
...
void show_progress_ivfdec(int frame_in, int frame_out, unsigned long dx_time)
{
    fprintf(stderr, "%d decoded frames/%d showed frames in %lu us (%.2f fps)\r",
            frame_in, frame_out, dx_time,
            (float)frame_out * 1000000.0 / (float)dx_time);
}
void generate_filename_ivfdec(const char *pattern, char *out, size_t q_len,
    ...
    do
    {
        ...
        if (p == next_pat)
        {
            ...
            case '9':
                ...
                break;
            default:
                die_ivfdec("Unrecognized pattern %%%c\n", p[1]);
            }

            pat_len = strlen(q);

            if (pat_len >= q_len - 1)
                die_ivfdec("Output filename too long.\n");
...
int ivfdec(int argc, const char **argv_)
{
    ...
    for (argi = argj = argv; (*argj = *argi); argi += arg.argv_step)
    {
        ...
        if (arg_match(&arg, &codecarg, argi))
        {
            ...
            if (k >= 0)
                ...
                die_ivfdec("Error: Unrecognized argument (%s) to --codec\n",
                           arg.val);
        }
    ...
    for (argi = argv; *argi; argi++)
        if (argi[0][0] == '-' && strlen(argi[0]) > 1)
            die_ivfdec("Error: Unrecognized option %s\n", *argi);
    ...
    else if (file_is_raw_ivfdec(infile, &fourcc, &width, &height, &fps_den, &fps_num))
        input.kind = RAW_FILE;
    else
    {
    ...
    {
        generate_filename_ivfdec(outfile_pattern, outfile, sizeof(outfile) - 1,
                                 width, height, 0);
        out = out_open_ivfdec(outfile, do_md5);
    }
    ...
    /* Decode file */
    while (!read_frame_vpxdec(&input, &buf, &buf_sz, &buf_alloc_sz))
    {
        ...
        if (progress)
            show_progress_ivfdec(frame_in, frame_out, dx_time);
        ...
        if (!noblit)
        {
            if (img)
            {
                ...
                if (!single_file)
                {
                    ...
                    out_fn[len] = '\0';
                    generate_filename_ivfdec(outfile_pattern, out_fn, len - 1,
                                             img->d_w, img->d_h, frame_in);
                    out = out_open_ivfdec(out_fn, do_md5);
                }
                else if (use_y4m)
                    out_put_ivfdec(out, (unsigned char *)"FRAME\n", 6, do_md5);

                buf = img->planes[VPX_PLANE_Y];

                for (y = 0; y < img->d_h; y++)
                {
                    out_put_ivfdec(out, buf, img->d_w, do_md5);
                ...
                {
                    out_put_ivfdec(out, buf, (1 + img->d_w) / 2, do_md5);
                ...
                if (!single_file)
                    out_close_ivfdec(out, out_fn, do_md5);
            }
        }
...
fail:
    ...
    if (single_file && !noblit)
        out_close_ivfdec(out, outfile, do_md5);
