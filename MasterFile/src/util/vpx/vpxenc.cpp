...
int stats_open_file_ivfenc(stats_io_t *stats, const char *fpf, int pass)
{
    int res;
...
int stats_open_mem_ivfenc(stats_io_t *stats, int pass)
{
    int res;
    stats->pass = pass;
...
void stats_write_ivfenc(stats_io_t *stats, const void *pkt, size_t len)
{
    if (stats->file)
    {
...
vpx_fixed_buf_t stats_get_ivfenc(stats_io_t *stats)
{
    return stats->buf;
}
...
int ivfenc(int argc, const char **argv_)
{
    ...
    for (argi = argj = argv; (*argj = *argi); argi += arg.argv_step)
    {
        ...
        if (arg_match(&arg, &codecarg, argi))
        {
            ...
            if (k >= 0)
                ...
                die_ivfenc("Error: Unrecognized argument (%s) to --codec\n",
                           arg.val);
        ...
        else if (arg_match(&arg, &passes, argi))
        {
            ...
            if (arg_passes < 1 || arg_passes > 2)
                die_ivfenc("Error: Invalid number of passes (%d)\n", arg_passes);
        }
        else if (arg_match(&arg, &pass_arg, argi))
        {
            ...
            if (one_pass_only < 1 || one_pass_only > 2)
                die_ivfenc("Error: Invalid pass selected (%d)\n", one_pass_only);
        }
    ...
    */
    if (one_pass_only)
    {
        ...
        if (arg_passes == 2 && !stats_fn)
            die_ivfenc("Must specify --fpf when --pass=%d and --passes=2\n", one_pass_only);
    }
    ...
    for (argi = argv; *argi; argi++)
        if (argi[0][0] == '-' && argi[0][1])
            die_ivfenc("Error: Unrecognized option %s\n", *argi);
...
#define SHOW(field) fprintf(stderr, "    %-28s = %d\n", #field, cfg.field)
        ...
        if (stats_fn)
        {
            if (!stats_open_file_ivfenc(&stats, stats_fn, pass))
            {
        ...
        {
            if (!stats_open_mem_ivfenc(&stats, pass))
            {
                fprintf(stderr, "Failed to open statistics store\n");
                return EXIT_FAILURE;
            }
        ...
        if (pass)
        {
            cfg.rc_twopass_stats_in = stats_get_ivfenc(&stats);
        }
